...
define(function (require) {
	...
	function getImageMeta(file, callback) {
		...
		r.onload = function (event) {
			if (file.type === "image/jpeg"||file.type=="") {//google nexus can't get the file name
				try {
	...
	function compress(file, picParam, callback) {
		...
		// defautl config
		var param = U.fn.extend(picParam, {
			type: "image/jpeg",
			maxHeight: 800,
			maxWidth: 600,
			quality: .8
		...
		getImageMeta(file, function (err, meta) {
			...
			if (meta && meta.tiff && meta.tiff.Orientation) {
				param = U.fn.extend(param, {orientation: meta.tiff.Orientation.value});
			}
			var canvas = document.createElement('canvas');
			mpImg.onrender = function () {
				var base64Str = "";
				if (U.ua.android && param.type == "image/jpeg") {
	...
	return {compress: compress,getImageMeta:getImageMeta};
});
